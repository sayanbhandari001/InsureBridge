    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Process and track reimbursement claim settlements with full billing breakdown.</p>
        </div>
            ...
            <CardContent className="p-5">
              <div className="flex flex-col lg:flex-row gap-5">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">{s.hospitalName} {s.admissionDate && s.dischargeDate && `· ${formatDate(s.admissionDate)} – ${formatDate(s.dischargeDate)}`}</p>

                  {/* Billing Breakdown */}
                  <div className="bg-muted/30 rounded-xl p-4">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Billing Breakdown</p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm mb-3">
                      {s.roomRentCharges && <div><p className="text-xs text-muted-foreground">Room Rent</p><p className="font-medium">{formatCurrency(s.roomRentCharges)}</p></div>}
                      {s.surgeryCharges && <div><p className="text-xs text-muted-foreground">Surgery</p><p className="font-medium">{formatCurrency(s.surgeryCharges)}</p></div>}
                      {s.medicineCharges && <div><p className="text-xs text-muted-foreground">Medicines</p><p className="font-medium">{formatCurrency(s.medicineCharges)}</p></div>}
                      {s.diagnosticCharges && <div><p className="text-xs text-muted-foreground">Diagnostics</p><p className="font-medium">{formatCurrency(s.diagnosticCharges)}</p></div>}
                      {s.otherCharges && <div><p className="text-xs text-muted-foreground">Others</p><p className="font-medium">{formatCurrency(s.otherCharges)}</p></div>}
                    </div>
                    <div className="border-t border-border pt-3 grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                      <div><p className="text-xs text-muted-foreground">Total Bill</p><p className="font-bold text-foreground">{formatCurrency(s.totalBillAmount)}</p></div>
                      {s.admissibleAmount && <div><p className="text-xs text-muted-foreground">Admissible</p><p className="font-bold text-green-700">{formatCurrency(s.admissibleAmount)}</p></div>}
                      {s.deductible && <div><p className="text-xs text-muted-foreground">Deductible</p><p className="font-medium text-red-600">{formatCurrency(s.deductible)}</p></div>}
                      {s.coPayAmount && <div><p className="text-xs text-muted-foreground">Co-pay</p><p className="font-medium text-amber-600">{formatCurrency(s.coPayAmount)}</p></div>}
                      {s.nonAdmissibleAmount && <div><p className="text-xs text-muted-foreground">Non-admissible</p><p className="font-medium text-red-600">{formatCurrency(s.nonAdmissibleAmount)}</p></div>}
                      {s.netPayableAmount && <div><p className="text-xs text-muted-foreground">Net Payable</p><p className="font-bold text-primary text-base">{formatCurrency(s.netPayableAmount)}</p></div>}
                    </div>
                    ...
                    {s.status === "paid" && s.utrNumber && (
                      <div className="mt-3 pt-3 border-t border-border flex items-center gap-2 text-xs">
                        ...
                        <span className="text-muted-foreground">Payment via {s.paymentMode?.toUpperCase()} · UTR: <span className="font-mono text-muted-foreground">{s.utrNumber}</span></span>
                      </div>
                    )}
                  </div>
                </div>
