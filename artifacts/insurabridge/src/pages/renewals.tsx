...
function PolicyChainBadge({ oldPolicy, newPolicy }: { oldPolicy: string; newPolicy: string | null }) {
  ...
  return (
    <div className="flex items-center gap-1.5 mt-2 flex-wrap">
      <span className="text-[10px] text-muted-foreground/70 font-medium uppercase tracking-wide">Policy Chain</span>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-foreground">Policy Renewals</h1>
          <p className="text-muted-foreground mt-1">Track and manage upcoming policy renewals.</p>
        </div>
            ...
            <CardContent className="p-5">
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-3 flex-wrap">
                    <h3 className="font-semibold text-foreground">{renewal.customerName}</h3>
                    <span className="font-mono text-xs text-muted-foreground bg-muted/40 px-2 py-0.5 rounded">{renewal.policyNumber}</span>
                    <StatusBadge status={renewal.status} />
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-sm">
                    <div>
                      <p className="text-xs text-muted-foreground">Insurer</p>
                      <p className="font-medium text-foreground">{renewal.insurerName}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Expiry</p>
                      <p className="font-medium text-foreground">{formatDate(renewal.expiryDate)}</p>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4 mt-2 flex-wrap">
                    {renewal.renewalDate && (
                      <p className="text-xs text-muted-foreground/70">Renewed: {formatDate(renewal.renewalDate)}</p>
                    )}
                    <button
              ...
              {completingId === renewal.id && (
                <div className="mt-4 pt-4 border-t border-border/50">
                  <p className="text-xs font-semibold text-muted-foreground mb-2">Enter the new policy number for the renewed policy:</p>
                  <div className="flex gap-2">
                    <input
                      value={newPolicyNum}
                      onChange={e => setNewPolicyNum(e.target.value)}
                      placeholder={`e.g. ${renewal.policyNumber.replace(/(\d{4})$/, m => String(parseInt(m) + 1))}`}
                      className="flex-1 px-3 py-2 text-sm font-mono rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-green-500/30"
                    />
                    <button
