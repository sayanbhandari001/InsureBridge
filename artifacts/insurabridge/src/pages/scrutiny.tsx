  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-slate-900">Scrutiny & Settlements</h1>
          <p className="text-slate-500 mt-1">Review and settle medical bills with deduction analysis.</p>
        </div>
        <button
      ...
      <div className="relative max-w-sm">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by patient or claim..." className="w-full pl-9 pr-4 py-2 text-sm rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/30" />
      </div>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr key={sc.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  <td className="px-4 py-3 font-medium text-slate-900">{sc.patientName}</td>
                  <td className="px-4 py-3 font-mono text-slate-600">{sc.claimNumber ?? "—"}</td>
                  <td className="px-4 py-3 text-slate-700">{formatCurrency(sc.billAmount)}</td>
                  <td className="px-4 py-3 text-slate-700">{sc.scrutinizedAmount ? formatCurrency(sc.scrutinizedAmount) : "—"}</td>
                  ...
                  <td className="px-4 py-3 text-slate-600">{sc.assignedTo ?? "—"}</td>
      ...
      {createOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <h2 className="text-lg font-bold text-slate-900 mb-5">New Scrutiny Case</h2>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-medium text-slate-700 mb-1 block">Bill Amount (₹)</label>
                  <input name="billAmount" type="number" required className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/30" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                ...
                <div>
                  ...
                  <input name="assignedTo" placeholder="Dr. Name" className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/30" />
                </div>
              </div>
              <div>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setCreateOpen(false)} className="flex-1 px-4 py-2 rounded-xl border border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50">Cancel</button>
      ...
      {reviewCase && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <h2 className="text-lg font-bold text-slate-900 mb-1">Review Case</h2>
            <p className="text-sm text-slate-500 mb-5">{reviewCase.patientName} — Bill: {formatCurrency(reviewCase.billAmount)}</p>
            <form onSubmit={e => handleComplete(e, reviewCase.id)} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-slate-700 mb-1 block">Scrutinized Amount (₹)</label>
                  <input name="scrutinizedAmount" type="number" required className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/30" />
                </div>
                <div>
                  <label className="text-xs font-medium text-slate-700 mb-1 block">Deductions (₹)</label>
                  <input name="deductions" type="number" className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/30" />
                </div>
              </div>
              <div>
                <label className="text-xs font-medium text-slate-700 mb-1 block">Deduction Reasons (comma-separated)</label>
                <input name="deductionReasons" placeholder="Reason 1, Reason 2..." className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <div>
                <label className="text-xs font-medium text-slate-700 mb-1 block">Remarks</label>
                <textarea name="remarks" rows={2} defaultValue={reviewCase.remarks ?? ""} className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none" />
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={() => setReviewId(null)} className="flex-1 px-4 py-2 rounded-xl border border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50">Cancel</button>
