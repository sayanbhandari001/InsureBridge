    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Telephonic communication records with summaries and decisions.</p>
        </div>
      ...
      {calls && calls.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Total Calls", value: calls.length, color: "text-slate-700" },
      ...
      <div className="space-y-3">
        {isLoading ? (
          <Card className="border-none shadow-sm p-8 text-center text-slate-500">Loading call logs...</Card>
        ) : calls?.length === 0 ? (
          <Card className="border-none shadow-sm p-12 text-center">
            <Phone className="w-12 h-12 text-slate-200 mx-auto mb-3" />
            <p className="text-slate-500">No call logs yet.</p>
          </Card>
        ) : calls?.map(call => {
          ...
          return (
            <Card key={call.id} className={`border-none shadow-sm overflow-hidden transition-shadow duration-200 ${isExpanded ? "shadow-md" : "hover:shadow-md"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <div className="flex items-center gap-1.5">
                      <span className="font-semibold text-slate-900">{call.callerName}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${ROLE_COLORS[call.callerRole] || "bg-slate-100 text-slate-600"}`}>
                        {ROLE_LABELS[call.callerRole] || call.callerRole}
                      </span>
                    </div>
                    <span className="text-slate-300">→</span>
                    <div className="flex items-center gap-1.5">
                      <span className="font-semibold text-slate-900">{call.receiverName}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${ROLE_COLORS[call.receiverRole] || "bg-slate-100 text-slate-600"}`}>
                  ...
                  {!isExpanded && call.notes && (
                    <p className="text-sm text-slate-500 truncate">{call.notes}</p>
                  )}
                </div>
              ...
              {isExpanded && hasDetail && (
                <div className="border-t border-slate-100 bg-slate-50/60 p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Summary */}
                  <div className="bg-white rounded-xl border border-blue-100 p-4">
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed">
                      {call.summary || call.notes || <span className="text-slate-400 italic">No summary recorded.</span>}
                    </p>
                  </div>

                  {/* Final Decision */}
                  <div className={`rounded-xl border p-4 ${call.finalDecision ? "bg-white border-emerald-100" : "bg-white border-slate-100"}`}>
                    <div className="flex items-center gap-2 mb-3">
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${call.finalDecision ? "bg-emerald-100" : "bg-slate-100"}`}>
                        <Gavel className={`w-3.5 h-3.5 ${call.finalDecision ? "text-emerald-600" : "text-slate-400"}`} />
                      </div>
                      <h4 className={`font-semibold text-sm ${call.finalDecision ? "text-emerald-900" : "text-slate-500"}`}>
                        Final Decision
                      </h4>
                    </div>
                    {call.finalDecision ? (
                      <p className="text-sm text-slate-700 leading-relaxed">{call.finalDecision}</p>
                    ) : (
          ...
          <form onSubmit={handleLogCall} className="space-y-4 mt-4">
            ...
            {user && (
              <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <div>
                  <p className="text-sm font-medium text-slate-800">Caller: {user.name}</p>
                  <p className="text-xs text-slate-500">{ROLE_LABELS[user.role]} · Logged in user</p>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label htmlFor="receiverName" className="text-sm font-medium text-slate-700">Receiver Name</label>
                <Input id="receiverName" name="receiverName" required placeholder="Dr. Smith / MediTPA..." />
              </div>
              <div className="space-y-1">
                <label htmlFor="receiverRole" className="text-sm font-medium text-slate-700">Receiver Role</label>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-1">
                <label htmlFor="direction" className="text-sm font-medium text-slate-700">Direction</label>
              ...
              <div className="space-y-1">
                <label htmlFor="outcome" className="text-sm font-medium text-slate-700">Outcome</label>
              ...
              <div className="space-y-1">
                <label htmlFor="duration" className="text-sm font-medium text-slate-700">Duration (secs)</label>
            ...
            {/* Summary */}
            <div className="space-y-1">
              <label htmlFor="summary" className="text-sm font-medium text-slate-700 flex items-center gap-2">
            ...
            {/* Final Decision */}
            <div className="space-y-1">
              <label htmlFor="finalDecision" className="text-sm font-medium text-slate-700 flex items-center gap-2">
