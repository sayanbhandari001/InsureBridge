...
function PolicyChainBadge({ oldPolicy, newPolicy, label }: { oldPolicy: string; newPolicy: string | null; label?: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5 mt-2 flex-wrap">
      <span className="text-[10px] text-muted-foreground/70 font-medium uppercase tracking-wide">{label ?? "Policy Chain"}</span>
      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-2xl shadow-2xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-border/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-foreground text-lg">Policy Lineage</h2>
              <p className="text-xs text-muted-foreground font-mono">{policyNumber}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-muted/40 rounded-xl transition-colors">
        ...
        <div className="overflow-y-auto flex-1 p-6">
          {isLoading ? (
            <div className="py-8 text-center text-muted-foreground text-sm">Loading lineage...</div>
          ) : !data?.events.length ? (
            <div className="py-8 text-center text-muted-foreground/70 text-sm">No linked events found for this policy.</div>
          ) : (
                ...
                <div key={`${ev.type}-${ev.id}`} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    ...
                    {idx < data.events.length - 1 && (
                      <div className="w-px flex-1 bg-muted/30 my-1" style={{ minHeight: 24 }} />
                    )}
                  </div>
                  <div className="pb-5 flex-1 min-w-0">
                    ...
                    {ev.type === "portability" ? (
                      ...
                      <p className="text-sm font-medium text-muted-foreground mb-1">{ev.insurerName}</p>
                    )}

                    <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                      <span className="font-mono text-xs bg-muted/40 text-muted-foreground px-2 py-0.5 rounded border border-border">{ev.policyNumber}</span>
                      {ev.newPolicyNumber && (
                        <>
                    ...
                    {ev.portabilityReason && <p className="text-xs text-muted-foreground mt-1 italic">Reason: {ev.portabilityReason}</p>}
              ...
              {data.events.length > 0 && (
                <div className="flex gap-4">
                  ...
                  <div className="pb-2 flex-1">
                    <p className="text-xs text-muted-foreground/70 pt-1.5">
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-foreground">Policy Portability</h1>
          <p className="text-muted-foreground mt-1">Manage customer requests to switch insurance providers.</p>
        </div>
            ...
            <CardContent className="p-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-mono text-muted-foreground text-xs">{req.policyNumber}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Current Sum</p>
                      <p className="font-medium text-foreground">{formatCurrency(req.sumInsured)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">New Sum</p>
                      <p className="font-medium text-foreground">{req.newSumInsured ? formatCurrency(req.newSumInsured) : "Same"}</p>
                    </div>
                  </div>
                  {req.portabilityReason && (
                    <p className="text-sm text-muted-foreground mt-2 italic">Reason: {req.portabilityReason}</p>
                  )}
                  ...
                  <div className="flex items-center gap-4 mt-2 flex-wrap">
                    <p className="text-xs text-muted-foreground/70">Requested: {formatDate(req.requestedAt)}</p>
                    <button
              ...
              {completingId === req.id && (
                <div className="mt-4 pt-4 border-t border-border/50">
                  <p className="text-xs font-semibold text-muted-foreground mb-2">Enter the new policy number issued by {req.toInsurerName}:</p>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setCompletingId(null)} className="px-3 py-2 rounded-xl border border-border text-muted-foreground text-sm hover:bg-muted/30">
                      Cancel
                    </button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 block">Reason for Portability</label>
                <textarea name="portabilityReason" rows={2} className="w-full px-3 py-2 text-sm rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none" />
              </div>
